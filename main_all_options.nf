...
/* snp_reads from HMM alignment are now aligned to megares
--- This is where RGI would go instead by using the snp_reads ---
process AlignDedupSNPToAMR {
     tag { sample_id }

     publishDir "${params.output}/DedupSNPAlignToAMR", mode: "copy"
     ...
     output:
         set sample_id, file("${sample_id}.amr.alignment.sam") into (dedup_snp_resistome_sam)
         set sample_id, file("${sample_id}.amr.alignment.dedup.bam") into (dedup_snp_resistome_bam)
...
process DedupRunResistome {
    tag { sample_id }

    publishDir "${params.output}/DedupSNPRunResistome", mode: "copy"

    input:
        set sample_id, file(sam) from dedup_snp_resistome_sam
        file annotation
        file amr

    output:
        file("${sample_id}.gene.tsv") into (dedup_snp_resistome, SNP_confirm_long)
...
/* nonhost reads now aligned to megares independently*/
